...
2. When a supported fair and bounded uncertainty exist, a nominal edge that does not comfortably clear the added uncertainty may resolve to zero-stake `PASS`, or `WAIT` only with its required independent signal and plausible actionability. If the fair/range cannot be supported after the required work, record a terminal evidence blocker. Continue unfinished work under shared scheduled authority section 6; if it remains `RESEARCH_INCOMPLETE` at delivery, identify it in the partial report while publishing other completed decisions. It is not an analytical PASS and does not veto unrelated decisions.
