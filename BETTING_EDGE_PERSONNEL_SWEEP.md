...
**Finding and materiality clarification — September 12:** retain the actual relevant starter names, expected lineup/rotation, material absences and supported status, with the source and time. "Lineup page checked" does not state what was found. For an incomplete official lineup, identify the exact player/role question and why a plausible outcome can change this moneyline, spread/run line/puck line or total. Use credible projections and current team/beat reporting on their existing terms. A blank final-lineup field alone does not make all markets personnel-unresolved; record `NO MATERIAL PERSONNEL SENSITIVITY` only when the actual reviewed facts support it. Do not remove a genuine dependency to obtain a passing gate.

Apply these named findings to any new or reused external forecast through the applicability review in `docs/FORECAST_EVIDENCE.md`. Unknown provider assumptions are not confirmation that the model includes today's starter or injury news. Preserve source forecast and prior check times; record current applicability checks separately. Shared event-level research may inform several cards, but its application and remaining limitation must be judged for each exact selection.
...
2. When a supported fair and bounded uncertainty exist, a nominal edge that does not comfortably clear the added uncertainty may resolve to zero-stake `PASS`, or `WAIT` only with its required independent signal and plausible actionability. Main reports may also complete the qualified non-wager market-assessment path under shared scheduled authority section 6 without an independent model/range, provided its actual material information requirements are met. A missing external forecast alone is not a blocker for that route. If neither permitted basis can be supported after the required work, record a terminal evidence blocker. Continue unfinished work under shared scheduled authority section 6; if it remains `RESEARCH_INCOMPLETE` at delivery, identify the specific missing fact, actual attempts and next action while publishing other completed decisions. It is not an analytical PASS and does not veto unrelated decisions.
