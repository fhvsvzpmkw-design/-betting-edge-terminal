...
> **NOT YET OPERATIONAL**
>
> This v0.6 document is a design draft. It is **not connected to the scheduler, Betting Edge report task, runner, or odds-refresh workflow**. It cannot affect a live report until a separate integration step is deliberately approved and tested.
...
# 21. V0.6 Shadow History & Decision Calibration Layer
...
V0.6 is designed as an extension of the existing Betting Edge flow:
...
## 21.4A Two-level identity model

Shadow History must distinguish the analytical thesis from the executable sportsbook quote.

### `candidateKey`

Represents the underlying analytical candidate and should be deterministic from material identity fields such as:

- event identity;
- market class;
- selection;
- line / threshold where applicable;
- regulation / period / full-game scope;
- player identity where applicable.

### `quoteKey`

Represents one executable quote and should bind:

- `candidateKey`;
- sportsbook;
- exact sportsbook market identity where available;
- quote timestamp;
- exact line / threshold;
- exact offered price.

The same `candidateKey` may therefore have multiple `quoteKey` records across Bet365, DraftKings, or multiple timestamps.

Line shopping occurs across compatible quotes for the same analytical candidate. Different thresholds or scopes must not be merged merely because the team/player/side is the same.
...
## 21.6A Normalized price-distance and edge measurement

Raw American-odds cents are useful for human display but must not be treated as a universal analytical distance measure.

Where calculable, Shadow History should preserve both the displayed American-odds difference and normalized quantities:

- current implied probability;
- fair implied probability;
- `playTo` implied probability;
- probability-point distance from `playTo`;
- estimated edge / EV margin after vig assumptions;
- later closing-line value in probability terms.

For two-outcome markets, implied-probability conversion should use the standard American-odds conversion before any no-vig normalization. For multi-outcome markets, the method used must be recorded.

Normalized metrics are calibration fields, not independent reasons to BET.
...
# 25. V0.6 Additional Regression Tests

Before any V0.6 feature becomes operational, add tests for:
...
# 26. V0.6 Open Research Questions
...
# 27. V0.6 Integration Plan — Not Yet Approved

## Phase A — Draft only

- maintain V0.6 as governance/specification;
...
# 28. V0.6 Compatibility Guard

V0.6 should be considered backward-compatible only if:
...
Any proposed implementation that violates these conditions requires a separate migration plan rather than being treated as a routine V0.6 integration.
...
V0.6 makes the operating architecture explicit.
...
Uploading `BETTING_EDGE_CONTRACT_DRAFT_v0.6.md` to the repository alone must not alter production behavior.
...
V0.6 distinguishes read permissions from write permissions.
...
# 34. V0.6 Integration Modes

V0.6 defines explicit activation modes so implementation can proceed incrementally.
...
# 35. V0.6 Compatibility Manifest and Cutover Tests

V0.6 is intended to integrate with the current Betting Edge architecture without radical replacement.
...
# 37. V0.6 Immutable Observation Contract

## 37.1 Original decision immutability

A Shadow History decision observation is a record of what Betting Edge knew and decided at a particular time.

After successful persistence, the original decision payload must be treated as immutable.

Do not rewrite original fields to incorporate:

- later sportsbook movement;
- later injury / lineup news;
- later weather information;
- closing prices;
- final results;
- hindsight-adjusted fair values;
- later contract or model revisions.

Corrections to objectively malformed identity/data may be represented by a superseding correction record that references the original observation. Silent mutation is not permitted.

## 37.2 Enrichment records

Post-decision information should be appended as separate enrichment records referencing the original `observationId`.

Recommended enrichment classes:
...
This keeps decision-time evidence auditable and prevents hindsight leakage.

---

# 38. V0.6 Idempotency, Retry, and Duplicate Contract

Every persisted decision observation should carry at minimum:
...
`observationId` must be deterministic or otherwise stable for the same logical issued observation.

If the writer receives the same `observationId` more than once:

- do not create a second logical decision record;
- verify whether the stored record matches the retry;
- treat an exact retry as idempotent success;
- surface a conflict if the same ID is associated with materially different content.

Manual reruns and workflow retries must therefore be distinguishable from genuinely new analytical decisions.

A new lane timestamp or explicitly new decision cycle may create a new `observationId` even for the same `candidateKey`.

---

# 39. V0.6 Shadow Record Lifecycle

A Shadow History candidate may progress through lifecycle states without rewriting its original decision:

1. `DECISION_RECORDED`
2. `MARKET_ENRICHED`
3. `EVENT_CLOSED`
4. `RESULT_ENRICHED`
5. `CALIBRATION_ELIGIBLE`

Lifecycle progression is descriptive. It must not imply that every enrichment source is always available.

A record becomes `CALIBRATION_ELIGIBLE` only when required decision-time identity and the calibration fields needed for the intended analysis are valid.

Incomplete records may remain stored for audit without being included in a particular calibration calculation.

---

# 40. V0.6 Degraded-Mode Matrix

The operating system should degrade by component rather than collapsing the entire report path unnecessarily.

| Component state | Report behavior | Shadow behavior | Calibration behavior |
|---|---|---|---|
| Authoritative contract unavailable | POLICY BLOCK for automated contract-dependent issuance | no new authoritative observation | no new readback |
| Live feed stale/unavailable | report may describe state; no price-dependent BET | may record failure/context if identity is valid | no current-price calibration claim |
| Exact serious-candidate quote unverified | no price-dependent BET for that candidate | record `PRICE_NOT_VERIFIED` where appropriate | exclude from executable-price analysis |
| Shadow writer unavailable | valid report continues | persistence failure surfaced | existing summaries remain read-only |
| Calibration summary unavailable | current analysis continues without historical readback | normal writes may continue | mark readback unavailable; do not invent history |
| Closing-price enrichment unavailable | no effect on issued report | defer enrichment | affected record not used for CLV analysis |
| Final-result enrichment unavailable | no effect on issued report | defer enrichment | affected record not used for outcome analysis |
| Runner unavailable after valid payload generation | analysis may still be valid but delivery is impaired | shadow behavior independent | calibration independent |

No degraded mode may silently substitute fabricated data.

---

# 41. V0.6 Interface Families

V0.6 defines four logical machine interfaces even before formal JSON Schema files exist.

## 37.1 Report interface

Represents the issued Betting Edge terminal payload and its audit metadata.

## 37.2 Shadow decision interface

Represents one immutable serious-candidate decision-time observation.
...
- lane / timestamp;
- event / sport / league;
- market / selection / threshold / scope;
- best verified executable quote and quote key where available;
- fair estimate / uncertainty;
- `playTo`;
- normalized edge / price-distance fields where available;
- status;
- controlling blocker;
- support / contrary summaries;
- historical-fit provenance;
- contract / model provenance.

## 37.3 Enrichment interface

Represents post-decision facts tied to an `observationId` without rewriting the decision.

## 37.4 Calibration-summary interface

Represents derived, versioned aggregates safe for controlled readback.
...
- filters / bucket definition;
- sample size;
- exclusions;
- contract/model cohorts where relevant;
- metric definitions;
- uncertainty / confidence information where practical.

---

# 42. V0.6 Calibration Eligibility and Cohort Integrity

Historical records created under materially different logic must not be naively pooled.

Calibration queries should be able to segment or control for:

- contract version;
- model / fair-value method version;
- sport / league;
- market class;
- threshold / price band;
- decision lane;
- known regime or rule change where relevant.

A larger heterogeneous sample is not automatically better than a smaller coherent sample.

The calibration layer may combine cohorts only when the combination is analytically defensible and documented.

---

# 43. V0.6 Machine-Readable Schema Promotion Rule

V0.6 describes the interfaces precisely enough to prototype them, but prose remains the governing draft until machine schemas are separately generated and validated.

Before passive production Shadow writes are enabled:

1. define a machine-readable decision schema;
2. define a machine-readable enrichment schema;
3. define a machine-readable calibration-summary schema;
4. validate representative good records;
5. validate rejection of malformed identity / status / stake / provenance fields;
6. version the schemas independently of contract prose where appropriate.

Schema validation must not be retroactively imposed on old historical artifacts unless a migration is deliberately performed.

---

# 44. Interface & Persistence Readiness Gate

The interface and persistence layer is considered sufficiently specified for production-readiness implementation when:

- the two-level identity model is unambiguous;
- original decision immutability is accepted;
- idempotent retry behavior is defined;
- normalized price / edge measurement is defined;
- lifecycle states are defined;
- degraded modes are explicit;
- interface families are defined;
- no new rule requires current production behavior to change merely by storing the draft.

---



# 45. V0.6 Production-Readiness Objective

V0.6 is the final pre-implementation specification target.

Its purpose is not to add new handicapping concepts. Its purpose is to remove operational ambiguity so implementation can be tested against a finite contract.

After V0.6, conceptual additions should normally be deferred unless testing reveals a real defect. The next source of learning should be implementation evidence.

V0.6 remains **non-operational while stored under a DRAFT filename**.

---

# 46. Authoritative Contract Manifest

Production authority must not depend on filename convention alone.

A future production system should use a small machine-readable manifest at an explicitly approved path, proposed as:

`BETTING_EDGE_CONTRACT_MANIFEST.json`
...
- immutable contract commit SHA or content hash where practical;
- activation mode;
- activation timestamp;
- optional rollback contract reference.

Draft files such as `BETTING_EDGE_CONTRACT_DRAFT_v0.6.md` must never become authoritative merely because they exist in the repository.

Changing the active manifest is a governance change and must not be performed by a live Betting Edge analytical run.

---

# 47. Proposed Production Repository Layout

The initial production layout should remain simple and backward-compatible.

Proposed paths:

- `BETTING_EDGE_CONTRACT.md` — approved human-readable contract;
- `BETTING_EDGE_CONTRACT_MANIFEST.json` — authoritative contract pointer / activation metadata;
- `schemas/shadow-decision.schema.json` — machine schema for immutable decisions;
- `schemas/shadow-enrichment.schema.json` — machine schema for post-decision updates;
- `schemas/calibration-summary.schema.json` — machine schema for derived summaries;
- `data/shadow-history/YYYY/MM/DD/<partition>.jsonl` — partitioned raw observations;
- `data/shadow-enrichment/YYYY/MM/DD/<partition>.jsonl` — partitioned enrichments;
- `data/calibration/` — compact derived summaries / manifests.

Existing production paths such as `data/live-odds.json`, runner files, and workflow files remain unchanged unless separately migrated.

A later move to a database must preserve these logical interfaces even if physical storage changes.

---

# 48. Exact V0.6 Shadow Decision Schema Contract

Before coding, the following logical fields are classified as required or conditional.

## 37.1 Required for every persisted decision

- `schemaVersion` — string;
- `observationId` — string, unique logical decision identifier;
- `runId` — string;
- `candidateKey` — string;
- `decisionTs` — offset-aware timestamp;
- `lane` — approved lane identifier;
- `sport` — string;
- `eventId` or stable event identity object;
- `marketType` — string;
- `selection` — string / stable selection identity;
- `status` — one of BET / LEAN / WAIT / PASS;
- `playTo` — valid threshold string/value under report semantics;
- `stake` — zero for every non-BET;
- `controllingReason` — controlled reason code / state;
- `contractVersion` — string;
- `contractRevision` — commit SHA/hash where available;
- `knowledgeCutoffTs` — timestamp equal to or not later than the decision context;
- `createdAt` — persistence timestamp.

## 37.2 Conditional execution fields

Required when a verified executable quote exists:
...
- implied probability;
- normalized `playTo` distance where calculable.

A record carrying `PRICE_NOT_VERIFIED` must not fabricate these fields.

## 37.3 Conditional analytical fields

Where defensible fair work exists:

- fair American price / probability;
- uncertainty representation;
- estimated edge / EV metric definition;
- `playTo` derivation context.

If fair work is unavailable, preserve `THRESHOLD UNAVAILABLE` behavior and no BET.

## 37.4 Required audit summaries for serious candidates

- strongest support;
- strongest contrary evidence;
- historical-fit grade or explicit unavailable state;
- historical-fit provenance or explicit insufficient-provenance state.

---

# 49. Exact V0.6 Enrichment Schema Contract

Every enrichment record must include:
...
- recorded timestamp;
- source / provenance;
- payload appropriate to the enrichment type.

Allowed initial enrichment types:
...
Enrichment must never change the original decision record.

If an enrichment is corrected, create a superseding enrichment record rather than silently mutating historical content whenever practical.

---

# 50. Exact V0.6 Calibration Summary Contract

A calibration summary may be consumed by live analysis only if it is:

- generated from schema-valid eligible observations;
- versioned;
- timestamped;
- based on a declared observation window;
- explicit about sample size;
- explicit about cohort filters;
- explicit about exclusions;
- explicit about metric definitions;
- free of decision-time leakage;
- approved for readback under the current activation mode.

Live analysis must be able to distinguish:

- summary unavailable;
- summary available but insufficient sample;
- summary advisory;
- summary approved for controlled readback.

No calibration summary may override hard price / identity / freshness gates.

---

# 51. V0.6 Contract-Driven Scheduler Interface

After MODE 2 activation, each scheduled job should resolve to a compact instruction containing only:

- lane identifier;
- lane label;
- trigger context;
- authoritative manifest path;
- expected contract activation mode;
- delivery requirement.

The contract supplies global rules. The lane definition supplies lane intent.

Before cutover, every production lane must pass old-prompt versus contract-driven equivalence tests on representative snapshots.

The existing self-contained prompts remain the rollback source until the contract-driven lane is accepted as the new known-good baseline.

---

# 52. V0.6 Read/Write Responsibility Matrix

| Component | Read | Write | Must not write |
|---|---|---|---|
| Scheduled report analysis | manifest, active contract, live odds, approved calibration | report output; optional handoff to shadow writer | contract, workflows, runner, scheduler definitions |
| Odds-refresh workflow | odds sources / secrets as currently approved | existing live-odds artifacts | contract / shadow governance |
| Shadow writer | validated shadow handoff, existing target partition metadata | approved shadow-history / enrichment paths | runner, contract, workflow definitions |
| Calibration builder | shadow decisions / enrichments | approved `data/calibration/` outputs | original shadow decisions |
| Runner | issued payload, supported live comparison data | local/presentation state as currently designed | analytical contract or historical records |
| Governance/operator path | draft/approved contract files and manifest | deliberate contract / manifest changes | automatic analytical mutation |

Permissions should be no broader than required by this matrix.

---

# 53. V0.6 Concurrency and GitHub Write Protocol

If GitHub is the initial persistence backend, writes must account for optimistic concurrency.

Required initial behavior:

1. determine target partition;
2. read current partition metadata/content or confirm absence;
3. validate pending record locally;
4. check `observationId` / `enrichmentId` for idempotency;
5. create or update using the current blob SHA where GitHub requires it;
6. on stale-SHA conflict, re-read the latest partition;
7. re-check duplicate/idempotency state;
8. retry only within a bounded policy;
9. if persistence still fails, preserve the valid report and surface shadow-write failure.

The writer must not overwrite unseen concurrent records.

Long-term storage may replace append-emulation on GitHub if write contention or file growth becomes operationally material.

---

# 54. V0.6 Partition and Size Policy

Initial partitioning should favor operational simplicity over maximum file density.

Default proposed partition:

`data/shadow-history/YYYY/MM/DD/<sport-or-lane>.jsonl`

Enrichment may use a parallel path.

Before production activation, implementation must define:

- maximum target partition size;
- rollover naming if the maximum is reached;
- whether sport or lane is the preferred partition axis;
- maximum retry count for concurrent updates;
- compaction policy, if any;
- integrity check / rebuild behavior.

No report should need to read raw partitions merely to perform routine current analysis.

---

# 55. V0.6 Production Cutover Protocol

Production cutover should occur in independently reversible steps.

## Gate 1 — Repository/document readiness

- V0.6 reviewed for contradictions;
- production contract file created separately from draft;
- manifest created but not activated;
- schemas generated and validated;
- no scheduler change yet.

## Gate 2 — Read-only contract test

- manual/test run reads manifest + contract;
- active contract identity verified;
- existing prompt remains authoritative;
- compare validation and analysis behavior.

## Gate 3 — Lane equivalence

- test each of 06:00, 08:00, 09:30, 15:15, 18:15;
- use representative fresh snapshots;
- verify hard gates, prices, statuses, stakes, risk, payload validity;
- investigate material differences.

## Gate 4 — MODE 2 authoritative read-before-run

- compact lane prompt becomes authoritative only after explicit approval;
- no Shadow History writes required;
- monitor several successful runs;
- rollback remains available.

## Gate 5 — MODE 3 passive Shadow History

- enable controlled writer only after write-access tests;
- confirm idempotency, duplicates, retries, stale-SHA handling, partition integrity;
- shadow failure cannot block reports;
- no readback yet.

## Gate 6 — Enrichment / calibration build

- populate closing/result enrichments;
- produce summaries;
- verify leakage controls and cohort integrity;
- summaries advisory only.

## Gate 7 — Controlled readback

- explicit approval required;
- historical influence remains subordinate to current price and hard gates;
- positive-effect monitoring continues.

---

# 56. V0.6 Acceptance Test Matrix

A production-ready implementation must demonstrate at minimum:

## Contract

- correct active version loaded;
- wrong hash/SHA rejected or blocked according to policy;
- missing manifest handled safely;
- draft contract cannot activate accidentally.

## Pricing

- fresh feed / fresh quote;
- fresh feed / stale quote;
- stale feed;
- exact-market mismatch;
- line shopping between Bet365 and DraftKings;
- no cached-search substitution.

## Decision semantics

- BET only when threshold and evidence clear;
- WAIT only when unresolved information could plausibly change actionability;
- PASS for current no-value states;
- EDGE GONE only after prior qualifying BET/LEAN on the same validated selection;
- every recommendation has valid `playTo`.

## Risk

- non-BET stakes forced to zero;
- total new risk exactly equals BET stakes;
- routine risk cap respected;
- duplicate exposure handled.

## Shadow persistence

- schema-valid record writes;
- duplicate retry is idempotent;
- conflicting duplicate surfaces error;
- concurrent stale-SHA retry preserves both records;
- failed write does not block valid report;
- original decisions remain immutable.

## Leakage / enrichment

- later information cannot rewrite decision-time inputs;
- closing market is separate enrichment;
- final result is separate enrichment;
- incomplete enrichments do not become silently calibration-eligible.

## Runner

- payload round-trip valid;
- same-snapshot repricing unchanged;
- lane switching intact;
- Clear Comparison intact;
- old reports remain readable.

---

# 57. V0.6 Rollback Plan

Rollback must be possible without deleting history or rewriting prior decisions.

Independent rollback controls:

- deactivate contract-driven scheduler and restore prior full prompts;
- point manifest to prior approved contract if appropriate;
- disable Shadow History writes;
- disable enrichment jobs;
- disable calibration summary generation;
- disable calibration readback;
- keep runner / odds refresh operating independently where their own gates pass.

Rollback does not erase already valid shadow records. Instead, later analysis records the version boundary.

---

# 58. V0.6 Promotion Boundary

V0.6 should be considered **implementation-ready** when:

- no unresolved ambiguity remains in identity, persistence, authority, permissions, fallback, or rollback semantics that would force an implementer to invent policy;
- machine schemas can be generated directly from the contract;
- the compact scheduler interface can be written without duplicating global rules;
- GitHub write behavior can be implemented with bounded concurrency handling;
- all activation gates have explicit pass/fail criteria;
- the existing working pipeline remains the rollback baseline.

V0.6 should not be called production V1.0 merely because the document is complete.

The next milestone after V0.6 is an implementation candidate / RC that proves the contract against real workflow behavior.

---

# 59. Freeze Principle Before RC

Once V0.6 enters implementation testing:

> **Prefer evidence-driven fixes over speculative feature additions.**

New conceptual features should be deferred unless they address a demonstrated implementation defect, safety issue, or material analytical gap.

This prevents perpetual drafting from delaying empirical validation.
