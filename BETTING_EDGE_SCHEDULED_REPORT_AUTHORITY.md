...
For new drafts, also read `docs/REPORT_CARD_EVIDENCE.md`, `docs/FORECAST_EVIDENCE.md`, `docs/CANDIDATE_ASSESSMENT.md` and `research/forecast-source-registry.json`. They govern card explanations, forecast retrieval and actual research application. The September 12 coverage and card checks are advisory; they add no report-wide publication gate or independent-model requirement for qualified market assessments. The September 16 candidate review is a per-selection completion requirement: use draft preparation to retain unfinished promising selections as `RESEARCH_INCOMPLETE` while completed decisions continue to publish.
...
The command prepares those two local draft files in place. Use the prepared files for the remaining advisory review, Core audit, normal validators and final bundle. Preparation runs `tools/forecast-evidence.mjs` and `tools/assemble-card-evidence.mjs` against the actual records; it cannot retrieve a missing forecast, supply a personnel finding, promote a decision or clear a blocker without the producer's completed assessment. From September 16, its candidate finalizer removes an incomplete promising evaluation from the issued-card draft and retains the real checks and next action in `RESEARCH_INCOMPLETE`; it recomputes draft counts and coverage. It never turns that unfinished work into PASS, LEAN, WAIT or BET. Review the deferred candidates and finish their assessment where supportable before final preparation. `review` and `candidates` are read-only. Preparation must never be used to rewrite frozen staging or issued history. The publisher verifies candidate completion on retained evaluations and can attach derived shortlist/test snapshots; it cannot rewrite frozen card decisions or evidence.
