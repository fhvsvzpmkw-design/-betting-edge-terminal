...
**Graham NFL fair handoff amendment:** `2026-09-21.1`, effective at/after `2026-09-21T06:00:00-07:00`. Read `docs/GRAHAM_NFL_FAIR_HANDOFF.md`. Shared candidate preparation now loads and pins the active Graham NFL spread board. Review the exact assessed quote, explicitly adopt/use as context/reject its dated fair or document unavailability, and retain the disposition and decision impact on the card. Resolve current information and disclosed limitations; incomplete selections remain unfinished under existing partial-publication rules. This supplies no calibrated probability, uncertainty band or automatic status change.
...
For new drafts, also read `docs/REPORT_CARD_EVIDENCE.md`, `docs/FORECAST_EVIDENCE.md`, `docs/CANDIDATE_ASSESSMENT.md` and `research/forecast-source-registry.json`. They govern card explanations, forecast retrieval and actual research application. General September 12 content-review warnings remain advisory and add no independent-model requirement for qualified market assessments. Beginning with the September 19 18:15 lane, exact structured-card identity is a narrow mechanical requirement: mismatched presentation evidence is detached and that selection remains `RESEARCH_INCOMPLETE` until rebuilt, while completed decisions continue to publish. The September 15 evening candidate review remains a per-selection completion requirement for other unfinished promising work.
...
Follow `docs/REPORT_CARD_EVIDENCE.md` while forming the draft. Create a fresh structured `cardEvidence` finding/application/limitation object for each new card so `tools/assemble-card-evidence.mjs` can retain the actual source references in SUPPORT, CONTRARY, ANALYSIS and SOURCE. Key the report recommendation, sidecar recommendation and EVALUATED receipt to the exact current `feed.selectionKey`; never reuse an object, array position, title or prior-card key as identity. Each finding must address that exact side, line and executable price, with its recorded stance and decision effect. Name recognizable publishers and readable Pacific check times. "Schedule checked" or "lineup page reviewed" records an attempt, not a substantive finding. Do not use an assembly loop that sets every matched selection to PASS, clears earlier forecasts, or substitutes generic process statements for completed research.
...
Before freeze, prepare the draft through the shared publication preparation path in section 9, then run `node tools/review-card-evidence.mjs review --report <prepared-report.json> --sidecar <prepared-sidecar.json>` and inspect its event facts and focused warnings. Correct the unfrozen draft from verified evidence and keep card/receipt copies aligned. Preparation and review do not invent missing research or certify source truth. The September 12 evidence warnings are advisory: preserve supported market assessments, retain truthful selection-level limitations and publish the completed remainder. The September 15 evening candidate completion requirement below applies separately. Historical-fit gaps do not block current decisions. The WAIT/BET requirements in section 6 remain unchanged.

### Candidate assessment — finish the strongest opportunities

From the candidate amendment, run `node tools/report-evidence-repair.mjs candidates --report <draft-report.json> --sidecar <draft-sidecar.json> --root <repo-root>` after binding the inventory and again after substantive research. The command ranks exact paired markets using the qualified price comparison and eligible exact forecast disagreements. It is a work queue, not an automatic pick list. Preserve both opposing selections and all available markets in the audit; the shortlist display is not a card cap.

Complete the strongest candidates first, with approaching event starts as an urgency consideration. Reuse one event/source check across relevant sides, then finish the exact market calculation. Record a source-linked `candidateAssessment` on each promising receipt: current quote binding; disposition of each eligible forecast; material personnel findings; assessment of the available LEAN and BET routes; a specific decision reason; and the price or non-price condition that would change the assessment. See `docs/CANDIDATE_ASSESSMENT.md` for the schema. A favorable comparison does not itself assign LEAN or BET. Adopt a model fair once per exact paired contract, with coherent opposing probabilities/ranges and the same assessment basis, then evaluate each execution price. Do not leave one side on a new model basis and its exact opposite on a conflicting market-only basis. Different book lines remain separate contracts.

For the market-reference route, an independently calibrated model interval is not a prerequisite to LEAN. Resolve the actual material information and consider the size, quality and contrary evidence for the price advantage. A small advantage may remain PASS with an explicit reason. For a forecast-based BET review, explain source applicability, accepted point estimate, source-grounded uncertainty, conservative-bound clearance and Core support. A provider's missing published interval starts the permitted range review; do not stop solely because the provider lacks an interval or because `directCalibration=GAP`. Never manufacture a range, independent support or an injury adjustment.

Reconcile personnel findings with the Core context and market information state. A blank final-lineup field alone is not a universal material dependency. If the relevant facts are confirmed, do not retain an unexplained `UNRESOLVED` Core flag. If a different material fact is still unresolved, name it and its impact. Genuine uncertainty may justify PASS, a properly qualified WAIT, or unfinished research; routine work still to do is not a WAIT.

After preparation, inspect the candidate review and continue the missing research while normal clocks permit. A promising draft decision with actual unanswered decision-sensitive work is deferred to its existing `RESEARCH_INCOMPLETE` receipt; this is not a negative handicap. Missing new metadata alone does not invalidate a completed, supported market assessment. The final report must distinguish issued PASS from unfinished candidate review. Completed selections retain their decisions and publish without waiting for the entire slate. Include the candidate shortlist, its remaining questions and the actual forecast-attempt count in the user delivery.
...
The command prepares those two local draft files in place. Use the prepared files for the remaining advisory review, Core audit, normal validators and final bundle. Preparation runs `tools/forecast-evidence.mjs` and `tools/assemble-card-evidence.mjs` against the actual records; it cannot retrieve a missing forecast, supply a personnel finding, promote a decision or clear a blocker without the producer's completed assessment. Beginning with the September 15 18:15 report, its candidate finalizer removes an incomplete promising evaluation from the issued-card draft and retains the real checks and next action in `RESEARCH_INCOMPLETE`; it recomputes draft counts and coverage. Beginning with the September 19 18:15 report, the same selection-level treatment applies to missing, mismatched or drifting `cardEvidence`: detach the presentation object, preserve the unissued decision for audit, and rebuild it against the exact selection before issuance. It never turns unfinished work into PASS, LEAN, WAIT or BET. Review the deferred candidates and finish their assessment where supportable before final preparation. `review` and `candidates` are read-only. Preparation must never be used to rewrite frozen staging or issued history. The publisher verifies candidate completion and exact card-evidence identity on retained evaluations; it cannot rewrite frozen card decisions or evidence.
