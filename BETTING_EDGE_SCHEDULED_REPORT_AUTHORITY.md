...
### Source-first fair-value workflow — from September 7 at 18:15 Pacific
...
Use these starting points, then search for another suitable exact-market forecast when necessary. They are research sources, not automatic endorsements, calibrated models or new executable books:

| Need | Source lookup |
|---|---|
| MLB moneyline | [FanGraphs scores](https://www.fangraphs.com/scores): select the operating date and read the pregame game-odds percentage, matched pitchers and posted/projected lineups. Do not use in-game win expectancy or season/playoff odds as a pregame game fair. |
| MLB total and run line; additional moneyline comparison | [Dimers MLB predictions](https://www.dimers.com/mlb/predictions): open the exact game and read the Moneyline, Over / Under or Run Line probability tab. Exact-line probabilities may also appear in its dated analysis. Record the forecast update time; distinguish model probability from the adjacent sportsbook price. |
| Other active sports/markets | Search established providers for an actual current model probability or explicitly stated fair handicap/total for the exact event and market. Apply the same provenance and personnel checks; MLB sources do not establish another sport's fair. |
