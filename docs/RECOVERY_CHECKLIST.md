...
This checklist is the mandatory completion gate for any manual report-lane recovery. It supplements `docs/OPERATIONS.md` and does not replace `BETTING_EDGE_CONTRACT.md` **v1.0**.
...
9. **Read back the publication.** Confirm the archived payload, sidecar, `run-history.json` entry, exact `run.ts`, `feedGeneratedAt`, feed blob provenance, lane and Contract v1.0 provenance all match. If any required history component is missing, the recovery is not finished.
...
Historical recoveries issued under Contract v0.9 remain immutable v0.9 evidence and are not relabeled by this checklist update.
