...
**Fair-construction clarification following the September 6 06:00 review:** follow the shared scheduled authority's section 6 workflow before assigning a blocker. A maintained/prebuilt model is not required for every market: a defensible `MARKET_ANCHORED_MODEL` with substantive independent current support remains permitted. Neither a source page lacking a ready-made probability nor `directCalibration=GAP` alone establishes that evaluation is impossible. Preserve the numeric fair, source-linked derivation, coherent opposing sides, independent-support and personnel requirements for every EVALUATED decision, including PASS.

Blocker attempts must explain the relevant handicapping work actually attempted and the specific obstacle. `CALIBRATION_UNAVAILABLE` means an identified calibration limitation prevents a defensible estimate or uncertainty range; name that limitation and explain why the permitted approach could not resolve it. `FAIR_MODEL_UNAVAILABLE` means the actual fair-construction work could not establish a defensible method, not merely that no stored model was found. Use `RESEARCH_INCOMPLETE` in the working draft for unfinished work and identify the remaining research. Record only real checks; a template that changes the event/market name while saying a schedule or pitcher page supplied no distribution is not a sufficient explanation. These distinctions use the existing receipt fields and reason codes; they do not convert blocked sides into decisions.

**Research completion — forward from `2026-09-06T18:15:00-07:00`:** the existing coverage and bundle validators reject `RESEARCH_INCOMPLETE` before freeze, publication, retries and read-back. Run `node tools/major-sport-market-coverage-gate.mjs research-plan --report <report.json> --sidecar <sidecar.json>` to derive the pending work from the bound inventory and existing draft. Continue it using shared scheduled authority section 6. The command is read-only, creates no evidence, and adds no per-game archive. Its `READY_FOR_VALIDATION` output is not publication approval. A genuine terminal blocker remains valid after applicable work/fallbacks; its reason, missing input, attempts and impact must explain why the fair/range still cannot be supported. An odds-file check is price evidence, not an independent research fallback. Do not relabel unfinished work to evade the check. If execution genuinely cannot continue, disclose `ANALYSIS INCOMPLETE — NOT PUBLISHED` with the observed cause and pending work, and do not stage READY. Earlier issued incomplete runs remain unchanged and validate under their original rules.
