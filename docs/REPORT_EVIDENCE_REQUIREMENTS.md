...
Blocker attempts must explain the relevant handicapping work actually attempted and the specific obstacle. `CALIBRATION_UNAVAILABLE` means an identified calibration limitation prevents a defensible estimate or uncertainty range; name that limitation and explain why the permitted approach could not resolve it. `FAIR_MODEL_UNAVAILABLE` means the actual fair-construction work could not establish a defensible method, not merely that no stored model was found. Use `RESEARCH_INCOMPLETE` for unfinished work and identify the remaining research. Record only real checks; a template that changes the event/market name while saying a schedule or pitcher page supplied no distribution is not a sufficient explanation. These distinctions use the existing receipt fields and reason codes; they neither create a new gate nor convert blocked sides into decisions.
