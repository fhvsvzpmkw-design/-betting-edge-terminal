...
## Card evidence application — September 12 repair; September 19 identity cutover
...
Supply the `cardEvidence` handoff documented in `REPORT_CARD_EVIDENCE.md` for new cards. Shared draft preparation attaches forecast coverage, then renders exact-market forecast conflicts, source finding/application/limitation, concrete WAIT reassessment conditions and canonical History Fit. Synchronize the recommendation, matching research record and completed receipt by exact `selectionKey`, never array position or title. General content-review warnings remain advisory. From `2026-09-19T18:00:00-07:00`, identity/parity is mechanically enforced: a mismatched object is detached and that selection remains `RESEARCH_INCOMPLETE` until rebuilt, without changing or inventing PASS/WAIT/LEAN/BET, fair, stake or source times. Other completed selections remain publishable.
...
PASS requires an actual supported conclusion, LEAN requires a favorable exact comparison with material information resolved, and WAIT keeps the existing independent-signal/actionability/trigger checks. None authorizes a wager. To issue BET, use the established fair/range path and clear all existing BET gates. Opposing selections must share the same paired reference or coherent model basis. The coverage gate re-resolves event, period, market, side, line, active pairing, freshness and arithmetic against the pinned observer; a producer-supplied QUALIFIED label alone is insufficient.

**Exact alternate-total amendment — from `2026-09-09T16:00:00-07:00`:** shared scheduled authority section 6 permits an exact Pinnacle full-game alternate total for a new zero-stake market assessment. `exactMarketReference` validates the raw pair even when the snapshot's unchanged default annotation requires a main line. Match the same numeric integer/half-point total and settlement, not a nearby total, quarter/split line, period, team total or alternate spread. Preserve timestamps and copy the resolver's `referenceKind: "EXACT_FULL_GAME_ALTERNATE_TOTAL"` and `bookmakerMarketId` into every mirrored `pinnacleBenchmark`. Bound validation checks both fields. Main-line assessments and pre-amendment history keep their previous interpretation. The baseline observer policy/hash stays unchanged; this is a report-time exception, not a retrospective reannotation.
