...
**Last updated:** 2026-09-06 — research completion repair following the 15:15 review
...
From the 18:15 September 6 completion amendment, shared authority section 6 provides an ordered qualified-odds → current-research → fair-value → decision working process and explicit fallback/continuation behavior. The existing coverage tool's read-only `research-plan` command derives pending event/market work from the exact inventory and existing draft receipts. Pre-freeze and publication validation now reject `RESEARCH_INCOMPLETE`; actual execution failure must be disclosed as ANALYSIS INCOMPLETE, without a READY bundle. Genuine terminal limitations remain possible after attempted research and alternatives. Contract benchmark/PASS wording, personnel decision wording and obsolete nine-card/schema curation guidance are reconciled with current primary-decision rules. No new research archive, numerical betting model, odds pull or schedule is introduced.

The 15:15 archived report remains valid historical evidence under its original timestamp rules. Verification must distinguish queue/gate behavior from research quality: the repair can reject unfinished work but cannot prove that cited facts or a fair model are sound. A fresh scheduled producer run completing real source research, an auditable fair/range and coherent decisions is still required to establish operational recovery. The working observation-freshness correction, Core uncertainty floors, numerical evidence and independent-support requirements remain in force.
