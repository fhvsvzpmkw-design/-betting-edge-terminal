...
**Draft version:** 0.8  
**Prepared:** 2026-08-15  
**Baseline contract:** `BETTING_EDGE_CONTRACT_DRAFT_v0.7(2).md`  
**Baseline contract SHA-256:** `17f37f132316678912c353b43e14249ba1636c51ee899b1664c8b7c0fe64d441`  
**Validated runner baseline:** `runner.html`  
**Validated runner SHA-256:** `6bb5ffa6e497069514bb35950394e1a5fec83af1c0e59ac2ff17ddb4762443df`  
...
> **NOT YET OPERATIONAL**
>
> This v0.8 document is a design draft and a surgical successor to the uploaded v0.7(2) baseline. It is **not connected to the scheduler, Betting Edge report task, runner, or odds-refresh workflow**. Uploading this draft alone cannot affect a live report until a separate integration step is deliberately approved and tested.
>
> v0.8 preserves the v0.7 historical-evidence architecture and all validated pricing/risk semantics except for three explicit additions: **authoritative repository/version preflight**, **deterministic equal-price sportsbook tie handling**, and **report timestamp integrity**.
...
## Invariant 8 — Authoritative baseline resolution before analysis

No Betting Edge analysis may begin until the run resolves the authoritative contract source and current runner source from the approved repository context.

For production contract-driven operation, use the approved production contract. During draft/manual testing, the exact draft must be explicitly named. **Do not infer authority from the numerically highest draft filename.**

If the repository/branch cannot be read, the intended contract cannot be unambiguously identified, the declared version conflicts with the intended version, or the current runner cannot be resolved, the run must stop at **PREFLIGHT BLOCK** before handicapping or live-price analysis begins.

A Library copy, remembered version, prior conversation artifact, or locally cached file must not silently substitute for the repository baseline. Manual recovery may use an alternate copy only when the user explicitly identifies the exact intended file/version and the substitution is disclosed in the run provenance.

---

# 4. HARD GATES

## 4.0 Repository / contract / runner preflight

Before any scheduled, manual, or ad-hoc Betting Edge analysis begins:

1. Resolve the approved repository target and approved branch (currently `main`).
2. Resolve the authoritative contract source:
   - production: the approved production contract/version policy;
   - draft/manual test: the exact draft explicitly named for the test.
3. Read the contract and verify that its declared version matches the intended version.
4. Resolve the current `runner.html` from the same approved repository context.
5. Record or retain enough provenance to identify the contract and runner actually used, preferably commit SHA and/or content hash when available.
6. Only after those checks pass may the run proceed to live-feed retrieval and handicapping.

### Authority rule

The existence of several draft files does not make the highest-numbered draft automatically authoritative. File recency, Library ordering, memory, or filename similarity must not decide contract authority.

### Failure behavior

If the repository is unavailable, version authority is ambiguous, the declared version is inconsistent, or the current runner cannot be resolved:

> **PREFLIGHT BLOCK — ANALYSIS NOT STARTED**

This is an operational preflight state, not a BET/LEAN/WAIT/PASS recommendation status.

Do not continue by silently substituting a Library copy or remembered contract. A user-directed manual recovery must explicitly identify the replacement artifact and must be disclosed.
...
### Equal-price tie rule

After exact-identity and executable-freshness filtering, if Bet365 and DraftKings offer the **same best executable price** for the same exact market/selection:

1. **Initial issuance:** prefer **Bet365**.
2. **Repricing:** if the originally issued book remains exact, fresh, and tied for the best executable price, retain the **issued book** to preserve same-snapshot stability and avoid meaningless book flips.
3. If the issued book is no longer eligible and the remaining best fresh price is tied, prefer **Bet365**.
4. If price and timestamp are identical, the rule above still governs; never rely on object order, encounter order, or sort stability.

The tie rule applies only after all identity and freshness gates pass. A stale Bet365 quote cannot win a tie against a fresh DraftKings quote, and sportsbook preference can never override a better fresh executable price.

If supported books expand in the future, any broader sportsbook-priority order requires deliberate change control.
...
7. Generate `run.ts` fresh at report-generation time in `America/Vancouver`.
8. Verify the local calendar date of `run.ts` matches the intended report date and the local calendar date of feed `generatedAt`. If they differ, validation fails.
9. Verify critical decoded fields.
10. Verify recommendation counts.
11. Verify every recommendation has an allowed status.
12. Verify every recommendation has `playTo`.
13. Verify every non-BET has zero stake.
14. Verify total risk equals BET stakes only.
15. Verify no price-dependent BET lacks verified price.
16. Verify no BET lacks defensible fair threshold.
17. Only then build/deliver the runner URL.
...
| Report `ts` date does not match intended report day and feed `generatedAt` date in Vancouver time | HARD FAIL | Regenerate `run.ts`; do not deliver runner link |
...
- known research gaps;
- where current-model uncertainty or model-error assumptions deserve scrutiny.

At R3 this is **advisory**. Research Library output may flag an uncertainty concern for explicit review, but it does not itself change a model-error margin, fair value, `playTo`, status, or stake.
...
## Change record — 2026-08-15 — v0.8 baseline, tie determinism, and timestamp integrity

### Issue 1 — Wrong-version analysis risk

A manual development session spent substantial time evaluating a lower-numbered side branch while a later contract lineage already existed. The failure mode was not handicapping logic; it was **authoritative-version resolution**.

### Decision 1

Add a repository/version preflight hard gate before analysis. Contract authority must be resolved from the approved repository context or from an explicitly named manual-test draft. Never infer authority from memory, Library ordering, or the numerically highest draft filename.

### Issue 2 — Equal-price book selection was order-dependent

The validated runner correctly filtered executable quote freshness before price ranking, but its equal-price result could depend on encounter/sort order. The 2026-08-15 09:30 regression fixture included a fresh Bet365/DraftKings exact-price tie on Texas at `-152`.

### Decision 2

Define deterministic tie behavior:

- initial issuance exact fresh-price tie → Bet365;
- same-snapshot reprice exact fresh-price tie → retain issued book if still eligible and tied;
- otherwise exact fresh-price tie → Bet365;
- never rely on encounter/object/sort order.

### Issue 3 — Report payload carried the wrong calendar date

A generated runner payload preserved a `run.ts` from the prior day even though the intended report and live feed were for the current Vancouver day. The runner correctly displayed the issued timestamp; the failure originated upstream in report generation.

### Decision 3

Add a timestamp-integrity hard gate at report generation:

- generate `run.ts` fresh in `America/Vancouver` immediately before payload serialization;
- never reuse or carry forward a prior run timestamp;
- require the Vancouver calendar date of `run.ts` to match the intended report date and feed `generatedAt`;
- any mismatch is a HARD FAIL and no runner link may be delivered until regenerated and revalidated;
- repricing preserves the issued `run.ts` and records later checks separately (for example `comparison.checkedAt` and feed `generatedAt`).

### Regression evidence

Using the uploaded v0.7(2) contract baseline and validated current runner:

- the corrected 09:30 Phillies executable quote selected fresh Bet365 `-135` over stale DraftKings `-130`;
- the corrected four-selection 09:30 fixture repriced against the identical snapshot with all matched prices **UNCHANGED**;
- non-BET stakes remained zero and total new risk remained `$0`.

### Intentionally unchanged

- 75-minute feed freshness threshold;
- 30-minute exact-quote freshness threshold;
- Bet365 + DraftKings as current supported books;
- fair-value framework;
- BET/LEAN/WAIT/PASS semantics;
- WAIT controlling-reason rule;
- EDGE GONE provenance restriction;
- staking/risk contract;
- Research Library historical-evidence architecture;
- Shadow History activation status;
- scheduler/canary timing;
- feed schema;
- payload schema;
- runner UI/history/instrumentation semantics except the explicit equal-price tie behavior when implemented.

---

## 13.1 No automatic governance mutation

A live Betting Edge report-generation run must never automatically rewrite or mutate:

- `BETTING_EDGE_CONTRACT.md` or any draft contract;
- workflow YAML;
- runner or index code;
- scheduler or canary definitions;
- Research Library artifacts;
- other governance or execution code.

Changes to those components require deliberate change control. Routine report generation may read approved inputs, but it must not gain broad repository mutation authority merely because another optional component may later require persistence.
...
- approved repo + `main` readable + explicitly intended contract resolves + declared version matches + current runner resolves → **PREFLIGHT PASS**;
- repo unavailable → **PREFLIGHT BLOCK**, analysis not started;
- multiple drafts present with no authoritative production contract and no explicitly named test draft → **PREFLIGHT BLOCK**;
- filename suggests one version but declared document version differs → **PREFLIGHT BLOCK**;
- Library/local copy available but repo read fails → no silent substitution;
- explicit user-directed manual recovery naming exact artifact → allowed only with disclosed provenance.
...
## Equal-price sportsbook ties

Test after exact identity and freshness filtering:

- Bet365 and DraftKings tied for best fresh price at initial issuance → Bet365 selected;
- issued DraftKings remains fresh/exact and tied with Bet365 on same-snapshot reprice → DraftKings retained;
- issued Bet365 remains fresh/exact and tied with DraftKings → Bet365 retained;
- issued book becomes stale/ineligible and remaining fresh books tie → Bet365 selected;
- one book has a genuinely better fresh price → better price wins regardless of preference;
- Bet365 stale and DraftKings fresh at same nominal price → DraftKings wins;
- repeated runs with identical inputs return the same book; no encounter/order-dependent output.

## 2026-08-15 09:30 acceptance fixture

The saved corrected 09:30 fixture must continue to satisfy:

- Phillies: stale DraftKings `-130` excluded; fresh Bet365 `-135` executable;
- Brewers: PASS semantics preserved;
- Vikings: WAIT only if pending information can plausibly cross `playTo`;
- Texas: fresh `-152`/`-152` tie follows the v0.8 tie rule;
- counts remain `0 BET / 1 LEAN / 1 WAIT / 2 PASS` when the Vikings WAIT materiality condition is accepted;
- all non-BET stakes `$0`;
- total new risk `$0`;
- identical-snapshot runner reprice returns UNCHANGED for all successfully matched issued prices.
...
- unresolved contract/runner authority → analysis not started;
...
## Phase 2 — Read-before-run / authoritative preflight

If approved, scheduled Betting Edge report generation must resolve the authoritative contract and current runner from the approved repository context before performing analysis.

The schedule itself remains only the trigger. If preflight cannot establish the intended contract version and runner source, the analysis does not begin.
...
1. Repository/version preflight passed and the intended contract + runner sources are auditable.
2. Approved feed route used.
3. Feed ≤75 minutes.
4. Serious executable quotes ≤30 minutes relative to snapshot.
5. Exact identity validated.
6. Best eligible supported user-book price used.
7. Any exact best-price tie followed the deterministic v0.8 book rule.
8. Fair value defensible.
9. BET evidence sufficiently strong and meaningfully independent.
10. Strongest contrary evidence considered.
11. Every rec has `playTo`.
12. Every non-BET has zero stake.
13. Total risk equals BET stakes.
14. Controlled vocabulary used correctly.
15. Payload passed round-trip validation.
16. `run.ts` was generated fresh in `America/Vancouver`, and its Vancouver calendar date matches the intended report date and feed `generatedAt`.
17. Zero BETs was permitted.
18. If R3+ is active, History Fit came from the approved Research Library process.
19. Personal ledger evidence did not silently alter the Research Library History Fit grade.
20. No adaptive discovery or research prior was incorrectly promoted into a hard rule.
...
- research caution against overly narrow uncertainty assumptions, with stricter local validation required before any current-model adjustment.
...
```json
{
  ...
  "bettingEdgeImplication": "Flag the absence of a universal prior; require local/book-aware validation and strict price verification. Do not auto-adjust current-model parameters at R3."
}
```
...
- thin props warrant a research caution against overly narrow model-error assumptions; any actual parameter change requires explicit current-model re-evaluation;
...
> **HISTORY FIT: NR — DIRECT RESEARCH IS THIN.** Broad peer-reviewed prop calibration remains limited, while book-specific vig and role/usage differences matter materially. Treat the library as a caution; any tighter model margin must come from the current-model process.
...
- favorite/longshot research may flag uncertainty around margin allocation for explicit review;
- market-efficiency research may raise the evidence burden before overriding a liquid consensus;
- thin-market research gaps may flag a need to re-check whether the current model is sufficiently conservative; the gap does not itself change model parameters.
...
- research caution against overly narrow model-error assumptions;
...
Low liquidity or integrity sensitivity should make History Fit caution that apparent movement may be less trustworthy. Any widening of current-model uncertainty requires explicit current-model justification rather than an automatic Research Library adjustment.
...
| Best near-term use | History Fit / uncertainty cautions / guardrails | Future local calibration |
...
# 52. V0.8 Activation Model — Three Independent Tracks

V0.6 used a mostly linear activation model that moved from contract documentation toward Shadow History writes.

V0.8 inherits the v0.7 **three independent tracks** because contract governance, Research Library integration, and Shadow History persistence remain separate capabilities.
...
- selected manual/test runs read V0.8;
...
- construct History Fit using the v0.7 historical-evidence rules inherited by v0.8;
...
V0.8 preserves the v0.7 sequence before revisiting Shadow History:
...
- provisional fair value, uncertainty/model-error parameters, `playTo`, status, and stake do not change solely because the History Fit grade or retrieved prior changes;
- if research triggers a re-evaluation, any changed recommendation is traceable to explicit current-evidence/current-model reasoning rather than the grade or prior itself;
- Research Library cannot manufacture a BET after a failed hard gate.

---

# 63. V0.8 Acceptance Test Matrix

## Contract

- draft has no production effect by upload alone;
- version correctly identifies 0.8;
- no live report run may automatically mutate contract/workflow/runner/scheduler/research governance artifacts;
- no section incorrectly states Shadow History is required for historical analysis;
- repository/version preflight blocks analysis when contract/runner authority is unresolved;
- a manual test draft is used only when explicitly named;
- report generation creates `run.ts` fresh in `America/Vancouver` and blocks delivery when its Vancouver date does not match the intended report date and feed `generatedAt`.
...
- equal best-price ties follow deterministic v0.8 Bet365/issued-book rules.
...
- at R3, History Fit does not silently rewrite fair value, uncertainty/model-error parameters, `playTo`, status, or stake.
...
- session switching/history/instrumentation remain intact;
- same-snapshot repricing remains UNCHANGED;
- equal-price initial issuance prefers Bet365;
- equal-price repricing retains the issued book when it remains fresh/exact/tied;
- fallback equal-price tie prefers Bet365;
- tie outcome is not encounter/order-dependent;
- repricing preserves the originally issued `run.ts` and records later comparison/feed timestamps separately;
- History Fit semantic update does not break rendering.

---

# 64. V0.8 Compatibility Manifest

V0.8 is intended as a surgical successor to v0.7(2) and to remain compatible with the current Betting Edge production architecture except for the explicitly approved repository-preflight, equal-price tie, and report-timestamp integrity rules.
...
- primary books (Bet365 + DraftKings remain unchanged; Bet365 gains explicit equal-price issuance/fallback priority);
...
- runner repricing semantics except deterministic retention of an eligible issued book on an exact best-price tie;
- bankroll handling;
- saved historical reports.

The inherited v0.7 conceptual change remains historical-evidence architecture. The v0.8 delta is deliberately narrow: authoritative baseline preflight, deterministic equal-price book selection, and report-timestamp integrity.
...
- review V0.8 for contradictions;
...
No V0.8 component should become authoritative merely because the document is detailed.
...
# 74. V0.8 Production-Readiness Objective

V0.8 is successful if Betting Edge can eventually do the following reliably:
...
That remains the inherited v0.7 historical-evidence correction; v0.8 adds authoritative-version resolution, equal-price execution determinism, and report-timestamp integrity.
...
# 76A. V0.8 Surgical Delta Summary

v0.8 inherits v0.7(2) unless this section or the 2026-08-15 v0.8 change record explicitly says otherwise.

The approved draft delta is limited to:

1. **Authoritative repository/version preflight before analysis** — resolve the intended contract and current runner from the approved repository context; fail closed when authority is ambiguous.
2. **Bet365-first initial equal-price ties** — after exact identity and freshness filtering, Bet365 wins an exact best-price tie at issuance.
3. **Issued-book retention on equal-price repricing** — if the issued book remains exact, fresh, and tied for best, keep it.
4. **Bet365 fallback on remaining equal-price ties** — when no eligible issued-book retention applies, Bet365 wins the exact fresh tie.
5. **No order-dependent ties** — encounter order, object order, and sort stability are never policy.
6. **Report timestamp integrity** — generate `run.ts` fresh in `America/Vancouver` immediately before payload serialization; require its Vancouver calendar date to match both the intended report date and feed `generatedAt`; fail closed and do not issue the runner link on any mismatch. Repricing must preserve the issued `run.ts` and use separate comparison/feed timestamps.

No other v0.7(2) rule is changed by this draft unless separately recorded through change control.

---

# 77. Final V0.8 Review Rule
...
- any path that permits analysis before authoritative contract/runner preflight passes;
- any equal-price selection path that depends on encounter/object/sort order;
- any report-generation path that reuses an earlier `run.ts` or allows the Vancouver report date, `run.ts` date, and feed `generatedAt` date to disagree.
...
V0.8 preserves the v0.7 lower-complexity path and adds only the minimum determinism required by the observed baseline/version, equal-price tie, and wrong-report-date failure modes:

> **Verify the right contract first; keep the live market primary; use the research we already have; make equal-price execution deterministic; stamp every new report with a fresh validated Vancouver timestamp; and postpone new persistence until it proves necessary.**
