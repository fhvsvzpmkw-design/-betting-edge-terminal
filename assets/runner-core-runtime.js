...
function quoteFromStructured(event,book,feedRef){const arr=event?.bookmakers?.[book];const wantedKey=String(feedRef.marketKey||'');let market=null;if(Array.isArray(arr)&&wantedKey)market=arr.find(m=>marketIdentity(m)===wantedKey)||null;if(!market&&feedRef.market)market=marketByName(event,book,feedRef.market);if(!market||!Array.isArray(market.odds))return null;const side=String(feedRef.side||'').toLowerCase();const label=normName(feedRef.label||'');const wantedSelection=String(feedRef.selectionKey||'');const hasHdp=feedRef.hdp!==undefined&&feedRef.hdp!==null&&feedRef.hdp!=='';const hdp=hasHdp?Number(feedRef.hdp):null;for(const row of market.odds){if(wantedSelection&&selectionIdentity(row,side,event,market)!==wantedSelection)continue;if(label&&rowLabel(row)!==label)continue;if(hasHdp){const line=rowLine(row);if(line===null||Math.abs(line-hdp)>0.001)continue}const dec=decimalOdds(row?.[side]);if(dec)return {book,dec,updatedAt:market.updatedAt,event,market:market.name,marketObj:market,side,row}}return null}
...
function quoteAgeMinutesAtFeed(q,feed){const qt=Date.parse(q?.updatedAt),ft=Date.parse(feed?.generatedAt);if(!Number.isFinite(qt)||!Number.isFinite(ft))return Infinity;return Math.max(0,(ft-qt)/60000)}
function bestBookQuote(quotes,feed,issuedBook=''){const fresh=(quotes||[]).filter(q=>quoteAgeMinutesAtFeed(q,feed)<=REPRICE_QUOTE_MAX_AGE_MINUTES);if(!fresh.length)return null;const bestDec=Math.max(...fresh.map(q=>q.dec));const tied=fresh.filter(q=>Math.abs(q.dec-bestDec)<1e-12);if(issuedBook){const retained=tied.find(q=>q.book===issuedBook);if(retained)return retained}return tied.sort((a,b)=>{const ai=BOOK_PRIORITY.indexOf(a.book),bi=BOOK_PRIORITY.indexOf(b.book);return (ai<0?999:ai)-(bi<0?999:bi)})[0]||null}
function buildInstrumentTelemetry(recs,feed){
  ...
  for(const rec of recs||[]){
    ...
    const a=quoteFromStructured(event,'Bet365',ref),b=quoteFromStructured(event,'DraftKings',ref);
...
function matchMoneyline(title,feed,books,ctx={},issuedBook=''){const m=title.match(/^(.*?)\s+(?:(?:3[- ]?way)\s+)?(?:ML|moneyline)(?:\s+(?:vs\.?|v\.?|at)\s+(.+))?$/i);if(!m)return null;const team=m[1].trim(),opponent=(m[2]||'').trim(),resolved=resolveTeamEvent(feed,team,opponent,ctx),event=resolved.event;if(!event)return {quote:null,reason:resolved.reason};const side=teamExact(event.home,team)||(!teamExact(event.away,team)&&teamLoose(event.home,team))?'home':'away';const quotes=[];let marketSeen=false;for(const book of books){const market=marketByName(event,book,'ML')||marketByName(event,book,'Moneyline');if(!market)continue;marketSeen=true;const row=market.odds?.find(x=>decimalOdds(x?.[side]));const dec=decimalOdds(row?.[side]);if(dec)quotes.push({book,dec,updatedAt:market.updatedAt,event,market:market.name,marketObj:market,side,row})}const quote=bestBookQuote(quotes,feed,issuedBook);return quote?{quote,reason:''}:{quote:null,reason:quotes.length?'PRICE NOT VERIFIED — QUOTE STALE':marketSeen?'PRICE NOT VERIFIED — BOOK PRICE MISSING':'MARKET UNAVAILABLE — ML NOT RETURNED'}}
function matchSpread(title,feed,books,ctx={},issuedBook=''){const m=title.match(/^(.*?)\s+([+-]\d+(?:\.\d+)?)(?:\s+(?:vs\.?|v\.?|at)\s+(.+))?$/i);if(!m)return null;const team=m[1].trim(),line=Number(m[2]),opponent=(m[3]||'').trim(),resolved=resolveTeamEvent(feed,team,opponent,ctx),event=resolved.event;if(!event)return {quote:null,reason:resolved.reason};const side=teamExact(event.home,team)||(!teamExact(event.away,team)&&teamLoose(event.home,team))?'home':'away';const quotes=[];let marketSeen=false,lineSeen=false;for(const book of books){const market=marketByName(event,book,'Spread');if(!market)continue;marketSeen=true;let rows=(market.odds||[]).filter(row=>{const rl=rowLine(row);if(rl===null)return false;if(Math.abs(Math.abs(rl)-Math.abs(line))>0.001)return false;lineSeen=true;return decimalOdds(row?.[side])});rows.sort((a,b)=>Math.abs((rowLine(a)??99)-line)-Math.abs((rowLine(b)??99)-line));const row=rows[0],dec=decimalOdds(row?.[side]);if(dec)quotes.push({book,dec,updatedAt:market.updatedAt,event,market:market.name,marketObj:market,side,row})}const quote=bestBookQuote(quotes,feed,issuedBook);return quote?{quote,reason:''}:{quote:null,reason:quotes.length?'PRICE NOT VERIFIED — QUOTE STALE':!marketSeen?'MARKET UNAVAILABLE — SPREAD NOT RETURNED':!lineSeen?'PRICE NOT VERIFIED — LINE NOT RETURNED':'PRICE NOT VERIFIED — BOOK PRICE MISSING'}}
...
function withoutComparison(run){const out=deepClone(run);if(!out)return out;delete out.comparison;delete out.refreshDelta;if(out.instrumentTelemetry?.authority!=='PUBLISHER_BOUND_FEED_V1')delete out.instrumentTelemetry;out.recs=(out.recs||[]).map(r=>{const x={...r};delete x.priceComparison;return x});return out}
...
function card(d,r){
  ...
  if(r.priceComparison){
    ...
    const values=[
      ...
      [pc.state==='MATCHED'?'PRICE TIME':closed?'STATE':'REASON',pc.state==='MATCHED'?vancouverClock(pc.updatedAt):txt(pc.reason,'NOT RETURNED')]
    ];
