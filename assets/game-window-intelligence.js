(()=>{
  ...
  function ensureStyle(d){
    ...
    style.textContent=`
      ...
      /* Odds/freshness returns to the header right; compact Reprice stays below Edge Readout. */
      #runnerLive .runnerHeadRight>.runnerFresh{margin:0!important}
      ...
      /* Keep the odds line anchored top-right on iPad portrait. */
  ...
  function compactRunnerUtilityLayout(){
    try{
      ...
      const headRight=head?.querySelector('.runnerHeadRight');
      ...
      if(fresh&&headRight&&fresh.parentElement!==headRight)headRight.appendChild(fresh);
      if(head)head.classList.remove('runnerUtilityDetached');
      if(refresh){
