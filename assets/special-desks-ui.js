...
function ensureStyle(d){
  ...
  const s=d.createElement('style');s.id=STYLE_ID;s.textContent=`
    ...
    .cryptoPick{--status:#9c87ff;position:relative;width:100%;border:2px solid #50456e;border-left:4px solid var(--status);background:#060812;padding:16px;box-shadow:inset 0 0 0 1px rgba(191,176,255,.025),0 0 12px rgba(0,0,0,.26);font-size:18px}
    ...
    .cryptoPickHead{display:flex;justify-content:space-between;gap:18px;align-items:flex-start}.cryptoPickIdentity{min-width:0}.cryptoStatusTag{display:inline-block;border:2px solid var(--status);color:var(--status);background:color-mix(in srgb,var(--status) 8%,#03060b);padding:6px 11px;font-size:16px;font-weight:950;letter-spacing:.10em;box-shadow:0 0 11px color-mix(in srgb,var(--status) 20%,transparent)}
    .cryptoPick h3{margin:10px 0 0;color:#f3f1fb;font-size:23px;line-height:1.2;letter-spacing:.055em;text-transform:uppercase}.cryptoMarket{margin-top:6px;color:#9187ad;font-size:14px;font-weight:900;letter-spacing:.08em}.cryptoEventMeta{display:flex;flex-wrap:wrap;gap:8px 18px;margin-top:10px;padding:10px 12px;border:1px solid #342a4d;background:#05060d;color:#aaa1bf;font-size:14px;font-weight:800;line-height:1.45;letter-spacing:.035em}.cryptoEventMeta span{white-space:normal}.cryptoEventMeta b{color:#cbbfff;font-size:11px;letter-spacing:.09em}.cryptoEventMeta .eventStart{color:#ece7ff}.cryptoEventMeta .eventStart b{color:var(--status)}.cryptoPickState{text-align:right;min-width:190px}.cryptoPickState small{display:block;color:#827899;font-size:12px;font-weight:950;letter-spacing:.11em;margin-bottom:5px}.cryptoPickState strong{display:block;color:var(--status);font-size:19px;font-weight:950;letter-spacing:.07em;line-height:1.35}.cryptoDecisionTag{display:inline-block;margin-top:8px;border:2px solid var(--status);color:var(--status);background:color-mix(in srgb,var(--status) 7%,#03060b);padding:5px 9px;font-size:15px;font-weight:950;letter-spacing:.07em}
...
function pizzaPanelHtml(data){
  ...
  const head=`<div class="specialHead pizzaHead"><div class="specialBadge pizzaBadge">${badge}</div></div>`;
...
function cryptoPanelHtml(data){
  ...
  return `<div class="specialHead cryptoHead"><div class="specialBadge cryptoBadge">${badge}</div></div>
...
function panelHtml(kind,data){
  ...
  return `<div class="specialHead cryptoHead"><div class="specialBadge cryptoBadge">${badge}</div></div>${items.length?`<div class="specialShelf">${items.map(x=>itemCard(x,kind)).join('')}</div>`:`<div class="specialEmpty"><strong>${emptyTitle}</strong><span>${emptyText}</span></div>`}`;
}
