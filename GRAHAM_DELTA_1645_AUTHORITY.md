...
Read `data/walters/nfl/qb-production/production-contract-v1.json`, `data/walters/nfl/qb-production-current.json`, `data/walters/nfl/qb-production-staging.json`, and `.github/workflows/graham-qb-performance-production.yml` before handling a quarterback case. Require production `state=OPERATIONAL_SCOPED`, authority token `APPROVED_WALTERS_QB_PERFORMANCE`, `productionAuthority=true`, `grahamWritesAllowed=true`, and `marketViewed=false`. Require exactly 32 team bindings and the approved team count/exclusions from the current production contract. The September 7 Atlanta scope amendment admits a documented value-invariant composite baseline; verify the matching applied amendment before using it.
