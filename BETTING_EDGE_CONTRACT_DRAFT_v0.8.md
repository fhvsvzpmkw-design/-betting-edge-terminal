...
>
> v0.8 preserves the v0.7 historical-evidence architecture and all validated pricing/risk semantics except for two explicit additions: **authoritative repository/version preflight** and **deterministic equal-price sportsbook tie handling**.
...
## Change record — 2026-08-15 — v0.8 baseline and tie determinism
...
16. Zero BETs was permitted.
17. If R3+ is active, History Fit came from the approved Research Library process.
18. Personal ledger evidence did not silently alter the Research Library History Fit grade.
19. No adaptive discovery or research prior was incorrectly promoted into a hard rule.
...
V0.8 is intended as a surgical successor to v0.7(2) and to remain compatible with the current Betting Edge production architecture except for the explicitly approved repository-preflight and equal-price tie rules.
...
The inherited v0.7 conceptual change remains historical-evidence architecture. The v0.8 delta is deliberately narrow: authoritative baseline preflight plus deterministic equal-price book selection.
...
That remains the inherited v0.7 historical-evidence correction; v0.8 adds authoritative-version and equal-price execution determinism.
...
V0.8 preserves the v0.7 lower-complexity path and adds only the minimum determinism required by the observed baseline/version and equal-price tie failure modes:

> **Verify the right contract first; keep the live market primary; use the research we already have; make equal-price execution deterministic; and postpone new persistence until it proves necessary.**
