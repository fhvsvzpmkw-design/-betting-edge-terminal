...
**Source-first fair-value amendment — forward from `2026-09-07T18:15:00-07:00`:** Betting Edge finds and assesses fair-value estimates from available published projections, source research and existing governed fair records, then compares them with the bound executable odds. A current, exact-market external model forecast can supply the independent fair; the report task need not own, reproduce or build that predictive model. Verify the source's basis, event, market/line, update time and personnel assumptions. Preserve the source estimate and attribute it; percentage/odds conversion is sufficient arithmetic when the source already supplies the required probability. An external page merely repeating sportsbook odds remains MARKET evidence. Do not originate a new predictive model or invent a personnel adjustment during a standard report run. Existing governed Walters/Graham authority remains applicable within its separate scope.

Stage 1 roster, lineup, starter, injury and availability research remains mandatory where material, with Stage 2 depth, fallback sources and final authoritative re-checks as required by section 6.3. Research must test whether the sourced estimate reflects the current matchup; seek an updated applicable projection when material facts change and avoid double-counting information already incorporated. Retain source-linked fair evidence, an explained uncertainty range and the actual Core support/error classification. Lack of an in-house model is not an evidence blocker. Shared scheduled authority section 6 and `docs/REPORT_EVIDENCE_REQUIREMENTS.md` govern sourcing, source disagreement, numerical conversion and remaining limitations. This dated amendment supersedes earlier standard-run directions to construct a new handicap from raw statistics; it does not change decision thresholds or permit unsupported PASS cards.

**Market-first completion amendment — forward from `2026-09-07T18:31:00-07:00`:** Main Betting Edge first grades exact executable prices against qualified market references, then checks current roster/lineup/news for every event, then performs a deeper investigation only for a material unresolved dependency or a question that may change the decision. The first price grade is provisional; personnel news can reopen any candidate. Shared scheduled authority section 6 and `docs/REPORT_EVIDENCE_REQUIREMENTS.md` define the authoritative workflow and structured market assessment. A verified market comparison plus current information review may complete an EVALUATED zero-stake PASS/LEAN/qualified WAIT without an independent model or numeric uncertainty range. Use `MARKET_DERIVED_ONLY`, clearly label the reference and preserve Core uncertainty; no invented fair, confidence interval or independent support is permitted. This supersedes the September 6 independent-fair requirement for these non-wager assessments and the earlier universal research-first delivery order. BET still needs the existing supported fair/range and all Core, personnel, price, staking and risk requirements. Pinnacle supplies a non-executable comparison; it does not set stake, overwrite an independent fair or directly change status. Existing issued reports remain immutable.

**Partial research publication from `2026-09-06T18:15:00-07:00`:** follow the working sequence and fallback process in shared scheduled authority section 6. Continue unfinished work, but preserve and publish every completed, validated decision even if other selections remain `RESEARCH_INCOMPLETE`. Retain actual attempts, missing inputs, the observed stopping reason and next steps for unfinished selections in the existing receipts, without a decision or stake. The publisher derives a PARTIAL REPORT notice and exact evaluated/unfinished counts; when zero selections were evaluated it discloses ANALYSIS INCOMPLETE instead of manufacturing PASS cards or claiming a no-value board. Genuine terminal blockers remain distinguishable. READY authorizes publication of a valid bundle with honest partial accounting; it does not require universal research completion. This replaces PR #66's whole-report veto before its first applicable run. Earlier issued reports remain immutable. No new archive or weaker decision evidence is authorized.
...
1. Complete the governed major-sport inventory sweep and each published decision's required research. From the partial research publication amendment, publish completed, validated decisions with explicit unfinished-selection accounting when some research remains incomplete.
...
For report timestamps at or after `2026-09-05T14:00:00-07:00`, section 6.1b supersedes this disappearance-only movement procedure. Earlier reports retain this historical procedure.
...
## 6.1a Full-game total movement — all covered sports
...
6. For a **prospective new bet**, Over benefits from a lower total and is worsened by a higher total; Under benefits from a higher total and is worsened by a lower total. Use LINE MOVED IN FAVOR, LINE MOVED AGAINST, or LINE UNCHANGED. This describes current betting terms, not closing-line value on an already placed ticket.
7. Compare odds separately using the archived issued odds and the current exact quote: PRICE IMPROVED (higher decimal payout), PRICE WORSENED (lower payout), PRICE UNCHANGED, or PRICE COMPARISON UNAVAILABLE when the old price is not safely parseable. These labels describe payout terms only; when the line also changes they do not measure the bet's net value. Show both old/new totals and available old/new odds in rec.move. MOVEMENT UNCHANGED may replace the two unchanged labels when both the total and odds are unchanged. Keep FAIR VALUE CHANGED separate when new analysis changes the estimate.
8. Reassess current fair value, uncertainty, playTo and final BET/LEAN/WAIT/PASS, including the existing personnel process where material. A favorable line move alone creates no BET. Non-BET stakes remain zero. If no current primary quote is verifiable, keep the candidate explicitly unavailable/unverified at zero stake; never show an old quote as currently executable. Use MARKET UNAVAILABLE for actual primary-market absence, and PRICE NOT VERIFIED for stale, ambiguous or identity-unresolved data.
9. Run tools/total-lineage.mjs audit before freezing/staging. Publication repeats that gate, including after rebasing on main and at remote read-back. The gate reads immutable History and emits the comparison diagnostics; it does not change analytical decisions, odds acquisition, schedules, prices, staking or issued records. Player props remain paused under the separate active report-scope authority.

## 6.1b Primary spread, run-line and puck-line movement — all covered sports
...
5. Independently show the prior/current issued and executable odds with PRICE IMPROVED, PRICE WORSENED, PRICE UNCHANGED or PRICE COMPARISON UNAVAILABLE under section 6.1a's payout comparison. Check this even when the handicap and selectionKey are unchanged. MOVEMENT UNCHANGED is valid only when both handicap and odds are unchanged. These are prospective betting terms; they do not establish net value or closing-line value on an existing wager.
6. One fresh supported book can supply the current primary quote. An absent, stale, ambiguous or identity-unresolved second book does not automatically veto that valid quote. If fresh verified books have different primary handicaps, preserve CONFLICTING SIGNALS and each book's signed handicap and price. Never invent a consensus or mix one book's line with another's price; ordinary Core uncertainty/price-quality gates still apply.
7. Freshly assess fair value, uncertainty, playTo and BET/LEAN/WAIT/PASS at the current line and odds, with the existing personnel research and Core gates. Keep informational FAIR VALUE CHANGED separate. A changed handicap is a new current selection and carries no automatic prior fair, status or stake. Non-BET stakes are zero. If no primary quote is verifiable, retain an explicit zero-stake unavailable/unverified decision with no executable old price.
8. Run tools/spread-lineage.mjs audit before freeze with the exact bound feed and sidecar. Both publication workflows repeat the check after rebasing and at remote read-back. The spread and total commands share tools/primary-lineage.mjs so their quote, history, odds comparison and decision checks stay aligned. This changes no Core threshold, stake method, schedule, issued record or prop-scope authority; props remain paused.

## 6.1c Full-game moneyline quote and price continuity — all covered sports
...
**September 9 card-evidence application clarification:** new unfrozen Main drafts follow `docs/REPORT_CARD_EVIDENCE.md`. Communicate actual source findings, selection-specific support/contrary evidence, decision impact and canonical History Fit with supported research references or an honest NR/unavailable state. Retain earlier same-day forecast leads for current reassessment even when a later card used only a market reference. The pre-freeze card review is advisory and cannot change or block publication. This clarifies research use and explanation; it preserves the qualified market-assessment route, selection-level unfinished research, partial publication, existing WAIT/BET eligibility and all execution gates. Issued reports remain immutable.
...
From the market-first completion amendment, use shared scheduled authority section 6: price grade, event information review, targeted depth, final decision and the existing publication checks. Steps 3–6 below describe the retained forecast-based investigation when that depth is needed; they do not block a supported market assessment.
