...
function replaceOnce(text,from,to,label){
  ...
  assert(first>=0,`Cannot find ${label}`);
  assert(text.indexOf(from,first+1)<0,`Ambiguous ${label}`);
  return text.slice(0,first)+to+text.slice(first+from.length);
}
function replaceRegion(text,startMarker,endMarker,replacement,label){
  const start=text.indexOf(startMarker);
  assert(start>=0,`Cannot find ${label} start`);
  const end=text.indexOf(endMarker,start);
  assert(end>=0,`Cannot find ${label} end`);
...
function runNode(args){
  ...
  assert(result.status===0,`${process.execPath} ${args.join(' ')} failed with status ${result.status}`);
  return result;
}
function runNodeExpectFailure(args,fragments){
  const result=spawnSync(process.execPath,args,{encoding:'utf8'});
  const output=`${result.stdout||''}${result.stderr||''}`;
  process.stdout.write(output);
  assert(result.status!==0,`${process.execPath} ${args.join(' ')} unexpectedly passed`);
  for(const fragment of fragments) assert(output.includes(fragment),`Expected diagnostic fragment not found: ${fragment}`);
}

const reportFile='tools/report-publication.mjs';
...
const reportLoopStart='    for(let i=0;i<report.recs.length;i++){' ;
const reportLoopStartIndex=report.indexOf(reportLoopStart,report.indexOf('function validateSidecar'));
assert(reportLoopStartIndex>=0,'Cannot find validateSidecar recommendation loop');
const reportLoopEndMarker='\n    }\n  }\n  return sidecar;';
const reportLoopEndIndex=report.indexOf(reportLoopEndMarker,reportLoopStartIndex);
assert(reportLoopEndIndex>=0,'Cannot find validateSidecar recommendation-loop end');
const newRecommendationLoop=`    const recommendationErrors=[];
...
report=report.slice(0,reportLoopStartIndex)+newRecommendationLoop+report.slice(reportLoopEndIndex+'\n    }'.length);

const newVerifyIndex=`function verifyIndex(root,{targetTs}={}){
...
report=replaceRegion(report,'function verifyIndex(root,{targetTs}={}){','\n\nfunction expectValidationFailure',newVerifyIndex,'verifyIndex');
fs.writeFileSync(reportFile,report,'utf8');

const coreFile='tools/core-v14-publication-gate.mjs';
...
core=replaceOnce(
  ...
  'validateBundle signature'
);
const coreLoopStart='  for(let i=0;i<report.recs.length;i++){' ;
const coreLoopStartIndex=core.indexOf(coreLoopStart,core.indexOf('function validateBundle'));
assert(coreLoopStartIndex>=0,'Cannot find validateBundle recommendation loop');
const coreLoopEndMarker='\n  }\n  return {core14:true};';
const coreLoopEndIndex=core.indexOf(coreLoopEndMarker,coreLoopStartIndex);
assert(coreLoopEndIndex>=0,'Cannot find validateBundle recommendation-loop end');
const newCoreLoop=`  const recommendationErrors=[];
  for(let i=0;i<report.recs.length;i++){
...
core=core.slice(0,coreLoopStartIndex)+newCoreLoop+core.slice(coreLoopEndIndex+'\n  }'.length);

const newVerifyHistory=`function verifyHistory(root){
...
core=replaceRegion(core,'function verifyHistory(root){','\n\nfunction main(){',newVerifyHistory,'verifyHistory');
fs.writeFileSync(coreFile,core,'utf8');

// Existing tests and clean production history must stay green.
runNode(['tools/core-handicap-framework.mjs','self-test']);
runNode(['tools/core-v14-publication-gate.mjs','self-test']);
runNode(['tools/report-publication.mjs','self-test']);
runNode(['tools/report-publication.mjs','verify']);
runNode(['tools/core-v14-publication-gate.mjs','verify-history']);

// Regression: inject two defects into one sidecar for each verifier and require both to appear.
const reportSidecar='data/history/research-fit/2026-08-27/evening-152141.json';
const coreSidecar='data/history/research-fit/2026-08-28/late-181633.json';
const reportOriginal=fs.readFileSync(reportSidecar,'utf8');
const coreOriginal=fs.readFileSync(coreSidecar,'utf8');
try{
  const reportData=JSON.parse(reportOriginal);
  reportData.recommendations[1].waitQualification=null;
  reportData.recommendations[2].waitQualification=null;
  fs.writeFileSync(reportSidecar,JSON.stringify(reportData,null,2)+'\n','utf8');

  const coreData=JSON.parse(coreOriginal);
  coreData.recommendations[1].coreAssessment.modelErrorState='HIGH';
  coreData.recommendations[3].coreAssessment.betEligibleByModelError=false;
  fs.writeFileSync(coreSidecar,JSON.stringify(coreData,null,2)+'\n','utf8');

  runNodeExpectFailure(
    ['tools/report-publication.mjs','verify'],
    ['recommendation 2 Luis Lara over 1.5 hits','recommendation 3 Leonie Fiebich over 1.5 three-pointers']
  );
  runNodeExpectFailure(
    ['tools/core-v14-publication-gate.mjs','verify-history'],
    ['recommendation 2 Toronto Tempo moneyline','recommendation 4 Los Angeles Sparks moneyline']
  );
}finally{
  fs.writeFileSync(reportSidecar,reportOriginal,'utf8');
  fs.writeFileSync(coreSidecar,coreOriginal,'utf8');
}
runNode(['tools/report-publication.mjs','verify']);
runNode(['tools/core-v14-publication-gate.mjs','verify-history']);
console.log('HISTORY VERIFIER AGGREGATE-DIAGNOSTICS REGRESSION OK');
