...
export function totalLineageApplies(report) {
  return ms(report?.ts) !== null && ms(report.ts) >= ms(TOTAL_LINEAGE_FROM);
}
export function totalEventSide(rec) {
  const feed = rec?.feed;
  if (marketKey(feed) !== 'totals' || !feed?.eventId || !['over', 'under'].includes(feed?.side)) return null;
  const detail = rec?.coreAssessment?.context?.marketDetail;
  if (detail && !['full_game_primary_total', 'full_game_total'].includes(detail)) return null;
  return [feed.eventId, 'totals', feed.side].join('|');
}
export function totalMovement(side, priorLine, currentLine) {
  ensure(['over', 'under'].includes(side) && number(priorLine) !== null && number(currentLine) !== null, 'Total movement requires an exact side and both numeric lines');
  const change = currentLine - priorLine;
  if (Math.abs(change) <= EPSILON) return 'LINE UNCHANGED';
  return (side === 'under' ? change > 0 : change < 0) ? 'LINE MOVED IN FAVOR' : 'LINE MOVED AGAINST';
}
export function totalPriceMovement(priorPrice, currentPrice) {
...
export function primaryTotal(event, book, side, generatedAt) {
  const market = (event?.bookmakers?.[book] || []).filter(item => marketKey(item) === 'totals')
  ...
  const result = inspectPrimaryLineMarket(market, ['over', 'under']);
  ...
  if (parts.length !== 5 || parts[0] !== eventId(event) || parts[1] !== 'totals' ||
    parts[2] !== side || number(parts[4]) !== result.line) return { book, state: 'IDENTITY', updatedAt: market.updatedAt };
  return { book, state: 'OK', line: result.line, selectionKey: key, priceAmerican: american(Number(row[side])),
    priceDecimal: Number(row[side]), updatedAt: market.updatedAt, method: result.method };
}

function priorTotals(root, report) {
...
export function auditTotalLineage({ root = process.cwd(), report, sidecar = null, feed = null, feedFile = null }) {
  ensure(ms(report?.ts) !== null, 'Report ts is invalid');
  if (!totalLineageApplies(report)) return { ok: true, enforced: false, diagnostics: [], violations: [] };
  const tracked = priorTotals(root, report);
  ...
  for (const rec of report.recs || []) {
    const key = totalEventSide(rec);
    if (!key) continue;
    if (currentBySide.has(key)) violations.push(key + ': duplicate current full-game total side');
    currentBySide.set(key, rec);
  }
  for (const prior of tracked) {
    const old = prior.rec, key = totalEventSide(old), event = events.get(String(old.feed.eventId));
    ...
    if (!current) {
      ...
      violations.push(key + ': tracked total disappeared; preserve the game and Over/Under side and reconcile the current primary total');
      continue;
    }
    problems.push(...decisionProblems(current));
    const text = String(current.move || '').toUpperCase();
    const primary = BOOKS.map(book => primaryTotal(event, book, old.feed.side, feed.generatedAt));
    ...
    if (!usable.length) {
      ...
      if (!required.test([text, current.price].join(' '))) problems.push('must explicitly identify the unavailable/unverified current total');
      if (String(current.status).toUpperCase() === 'BET' || priceDecimal(current.price) !== null) problems.push('cannot issue a BET or an executable price without a verified current primary total');
    } else {
      const selected = usable.find(item => item.book === canonicalBook(current.book));
      if (!selected) problems.push('current book does not have a fresh verified primary total for this side');
      else {
        const currentLine = lineOf(current.feed);
        const suppliedLines = ['hdp', 'line', 'total', 'points'].filter(field => current.feed[field] !== undefined && current.feed[field] !== null);
        if (currentLine !== selected.line || suppliedLines.some(field => number(current.feed[field]) !== selected.line) ||
          current.feed.selectionKey !== selected.selectionKey) problems.push('current identity must match the selected book primary total, not an old or alternate line');
        ...
        const movement = oldLine === null ? 'PRIOR LINE UNVERIFIED' : totalMovement(old.feed.side, oldLine, selected.line);
        const priceMovement = totalPriceMovement(old.price, selected.priceAmerican);
        Object.assign(diagnostic, { state: movement, lineMovement: movement, priceMovement });
        const unchanged = movement === 'LINE UNCHANGED' && priceMovement === 'PRICE UNCHANGED' && text.includes('MOVEMENT UNCHANGED');
        if (!unchanged && (!text.includes(movement) || !text.includes(priceMovement))) problems.push('movement text must report ' + movement + ' and ' + priceMovement + ' separately');
        if (oldLine !== null && (!hasNumber(current.move, oldLine) || !hasNumber(current.move, selected.line))) problems.push('movement text must show the prior and current totals');
        if (priceMovement !== 'PRICE COMPARISON UNAVAILABLE' &&
      ...
      if (distinct.size > 1) {
        ...
        if (!text.includes('CONFLICTING SIGNALS')) problems.push('different book primary totals must be reported as CONFLICTING SIGNALS');
        for (const item of usable) if (!text.includes(item.book.toUpperCase()) || !hasNumber(current.move, item.line) || !hasNumber(current.move, Number(item.priceAmerican))) problems.push('preserve each conflicting book total and price');
      }
      if (/MARKET UNAVAILABLE|PRICE NOT VERIFIED/.test(String(current.price || '').toUpperCase())) problems.push('fresh supported primary total exists; it must not be presented as unavailable');
    }
    diagnostics.push(diagnostic);
