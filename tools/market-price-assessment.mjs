...
import {AUTHORITY, QUALIFIED, qualifyMarket} from './pinnacle-sharp-benchmark.mjs';
...
// Resolve the provider's full-game main line, never an alternate, team total or
// period with a similar outcome label. Existing qualification clocks are reused.
export function exactMarketReference(report, quote, observer) {
  ...
  for (const market of pinnacle?.markets || []) {
    ...
    if (selected && opposite) matches.push({fixtureId: fixture.fixtureId, bookmakerMarketId: market.bookmakerMarketId, selected: selected.row, opposite: opposite.row, generatedAt: observer.generatedAt});
  }
  ensure(matches.length === 1, 'Market assessment requires a qualified exact full-game paired main line');
  return matches[0];
}
